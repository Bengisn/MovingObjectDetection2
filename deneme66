...
tracks = {};
tentativeTracks = {};
for i = 1:1:40%numOfFramesInTheVideo
    if i <= numOfAveragedFrames
        ...
        if mod(i,numOfAveragedFrames) == 0
            ...
            %stats = blobMeasurements
            
            %             for x=1:length(stats)
            %                 %saving data
            %                 bb = stats(x).BoundingBox;
            %                 bc = stats(x).Centroid;
            %                 be = stats(x).Orientation; %yön
            %                 %draw the rectangle
            ...
            availabilityList = ones(1,numberOfObjects);% available 1, availbe değilse 0 olacak. Bunun için ilerde güncellenecek
            
            %             xVelocity = xUzunluk / (numOfAveragedFrames*videoObject.FrameRate);
            %             yVelocity = yUzunluk / (numOfAveragedFrames*videoObject.FrameRate);
            %
            if size(tracks,2) > 0
            ...
            for newTentTrackCounter = 1:1:numberOfObjects
                if availabilityList(newTentTrackCounter) == 1
                    ...
                    % gate i
                    tentativeTracks{1,size(tentativeTracks,2)}.gateX = tentativeTracks{1,size(tentativeTracks,2)}.x; % gate x
                    tentativeTracks{1,size(tentativeTracks,2)}.gateY = tentativeTracks{1,size(tentativeTracks,2)}.y; % gate y
            ...
            for k=1:length(stats)
                xUzunluk = abs(stats(k).BoundingBox(2) - stats(k).BoundingBox(1)); %dikdortgenin x uzunlugu
                yUzunluk = abs(stats(k).BoundingBox(4) - stats(k).BoundingBox(3)); %dikdortgenin y uzunlugu
                ortaNokta = stats(k).Centroid;
                
                %rectangle('Position', xUzunluk, 'EdgeColor', 'r', 'LineWidth', 2);
            end
