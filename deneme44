...
for i = 1:1:numOfFramesInTheVideo
    if i <= numOfAveragedFrames
        ...
        if mod(i,numOfAveragedFrames) == 0
            ...
            stats = regionprops(labeledImage,'BoundingBox', 'Centroid', 'Orientation'); %measure properties of image regions such as: 'Area', 'Centroid', 'Box'
            %stats = blobMeasurements
%             N = size(blobMeasurements,1); %number of objects in the image
            
            for x=1:length(stats)
                %saving data 
                bb = stats(x).BoundingBox;
                bc = stats(x).Centroid;
                be = stats(x).Orientation;
                %draw the rectangle
                rectangle('Position', bb, 'EdgeColor', 'r', 'LineWidth', 2);
%                 plot(blobMeasurements(x).Centroid(1), blobMeasurements(x).Centroid(2), 'ro');
%                 previousFrame = frame;
%                 frame = zeros(videoObject.Height,videoObject.Width,3);
            end
            pause(1)
...
%             for a=1:N
%                hold on 
%                centroid = blobMeasurements(a).Centroid;
%                C_X = centroid(1);
%                C_Y = centroid(2);
%                rectangle('Position', blobMeasurements(a).BoundingBox, 'EdgeColor', 'g', 'LineWidth', 2)
%                previousFrame = frame;
%                frame = zeros(videoObject.Height,videoObject.Width,3);
